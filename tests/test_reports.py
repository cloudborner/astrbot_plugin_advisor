...
class ReportTests(unittest.TestCase):
    ...
    def test_analysis_report_removes_markdown_html_and_internal_counts(self):
        ...
        self.assertLess(rendered.index("群号：123456789"), rendered.index("核心结论"))
        self.assertIn('class="footer report-footer"', rendered)
        self.assertIn('class="footer-assurance">推荐结果仅供参考', rendered)
        self.assertIn('class="footer-guidance">安装前请核对插件说明', rendered)
        self.assertIn('class="meta report-meta"', rendered)
        self.assertIn("grid-template-columns: minmax(0, 1fr)", rendered)
        self.assertIn("推荐结果仅供参考，不保证实际质量或适用性", rendered)
        ...
        self.assertLess(fallback.index("群号：123456789"), fallback.index("推荐结果仅供参考"))
        self.assertLess(fallback.index("推荐结果仅供参考"), fallback.index("安装前请核对插件说明"))
