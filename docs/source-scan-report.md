...
扫描时间：2026-08-24 17:51:38 UTC  
扫描模式：本地、离线、只读静态分析；未安装依赖，未导入或执行插件代码。

## 结果摘要

- 本地源码目录 1815 个，成功映射并扫描 1815 个；未映射、冲突和重复映射均为 0。
- 市场另有 19 个条目没有本地源码，运行时索引对其保留原有元数据画像。
- 源码画像：`data/source_resource_profiles.json`（1815 条）。
- 运行时画像：`data/source_resource_index.json`（1834 条，其中源码静态画像 1815 条、元数据回退 19 条）。
...
在 1815 个源码绑定插件中：上调 721、不变 860、下调 234。旧报告中由依赖名、模型文件名、Chrome/FFmpeg 文本和普通缓存造成的大量 L4 已被消除。
...
- 项目测试全集运行 135 项；另有 3 项因本机未安装项目依赖 `packaging` 无法导入，本次未安装依赖。
