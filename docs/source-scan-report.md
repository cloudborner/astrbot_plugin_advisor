...
- 功能证据：`data/source_function_evidence.json`（1810 条）；经定向复核的语义画像为 `data/source_function_llm_profiles_v3_reviewed.json`（1810 条），最终能力索引覆盖 1834 条。
...
- 正式能力索引已消费 1810 条复核语义画像；版本、源码摘要、字段结构和证据引用不匹配时会拒绝或回退，另外 24 条使用市场资料画像。
- 接入复核画像后，正式索引由约 3.22 MB 降至 2.06 MB，平均能力词由每插件 15.2 个降至 6.8 个，安装说明、帮助标题和普通配置项不再挤占推荐上下文。
- 运行时索引语义校验：1834/1834 个市场版本绑定，画像哈希有效。
- 画像公式、L0–L4 范围、人工队列标记和本地模型关键证据均通过一致性检查。
- 项目测试全集运行 280 项，1 项按环境设计性跳过，另有 30 个子测试通过；Ruff 与 compileall 通过。
