...
def visible_text(value: object, maximum: int = 500, *, preserve_newlines: bool = False) -> str:
...
def _report_copy(value: object, maximum: int, *, clauses: bool = False) -> str:
    """Keep authored lines; split reason clauses without breaking quoted evidence."""
    text = visible_text(value, maximum, preserve_newlines=True)
    if not clauses:
        return text
    depth = 0
    parts: list[str] = []
    for char in text:
        if char in "（(":
            ...
            depth = max(0, depth - 1)
        parts.append("\n" if char in "；;" and depth == 0 else char)
    return "\n".join(part.strip() for part in "".join(parts).split("\n") if part.strip())


def _escape_report_copy(value: object, maximum: int, *, clauses: bool = False) -> str:
    return html.escape(_report_copy(value, maximum, clauses=clauses), quote=True).replace("\n", "<br>")
...
def render_analysis_report_html(data: AnalysisReportData) -> str:
    needs = "".join(
        ...
        f'<div class="need-evidence">{_escape_report_copy(item.evidence, 140)}</div>'
        "</div>"
        "</article>"
...
.rec-name {{ min-width: 0; margin-right: auto; font-size: 23px; font-weight: 780; overflow-wrap: anywhere; }}
...
.rec-reason {{ min-width: 0; overflow-wrap: anywhere; font-size: 19px; line-height: 1.65; }}
...
<div class="hero-copy">{_escape_report_copy(data.conclusion, 220)}</div></div>
...
def analysis_report_text(data: AnalysisReportData) -> str:
    needs = "、".join(
        ...
        f"{'：' + _report_copy(item.evidence, 140) if item.evidence else ''}）"
        for item in data.needs[:3]
    ) or "暂未形成可靠需求"
    ...
    limitation = (
        f"\n说明：{_report_copy(data.limitation, 220)}" if data.limitation else ""
    )
    return (
        "群需求分析\n"
        f"核心结论：{_report_copy(data.conclusion, 220)}\n"
